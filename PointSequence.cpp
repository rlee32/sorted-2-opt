...
std::vector<Connection> PointSequence::connection_sequence() const
{
    std::vector<Connection> sequence;
    primitives::point_id_t i{0};
    while (sequence.size() < m_next.size())
    {
        auto next = m_next[i];
        sequence.emplace_back(i, next);
        i = next;
    }
    return sequence;
}

void PointSequence::new_tour(const std::vector<Connection>& old_connections
    , const std::vector<Connection>& new_connections)
{
    std::vector<Segment> old_segments;
    for (const auto& c : old_connections)
    {
        old_segments.emplace_back(c);
    }
    std::vector<Segment> new_segments;
    for (const auto& c : new_connections)
    {
        new_segments.emplace_back(c);
    }
